...
public class Regional
{
    ...
    {
        /* 1. Arranca el Communicator leyendo regional.cfg
              aquí debe estar Ice.Default.Locator cuando uses IceGrid */
        try (Communicator ic = Util.initialize(args, "regional.cfg")) {
            ...
            try {
                ...
                adapter = ic.createObjectAdapterWithEndpoints(
                             "RegionalAdapter", "tcp -h 0.0.0.0 -p 10000");
            }

            /* 3. Registra el servant y publica el proxy indirecto VoteCollector */
            ReceptorVotos servant = new ReceptorVotos();
            IRegistrarVotoPrx prx = IRegistrarVotoPrx.uncheckedCast(
                    adapter.add(servant, Util.stringToIdentity("VoteCollector")));

            System.out.println("RegionalServer listo en " + prx);   // log de utilidad

            adapter.activate();
            ic.waitForShutdown();   // bloquea hasta Ctrl-C o shutdown remoto
        }
    }
}
