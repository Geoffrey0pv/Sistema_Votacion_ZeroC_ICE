...
public class SistemaVerificacion {
    ...
    private void verificarBaseDatos() {
        ...
        if (!dbFile.exists()) {
            ...
            System.err.println("💡 Ejecute 'distribuir <departamento>' en el servidor regional primero");
            throw new RuntimeException("Base de datos de mesa no encontrada");
        }
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL);
            ...
            if (rs.next()) {
                ...
                estadisticas += "   📈 Porcentaje verificado: " + 
                    (totalVotantes > 0 ? String.format("%.2f%%", (double)votantesVerificados / totalVotantes * 100) : "0%") + "\n";
                estadisticas += "=====================================";
